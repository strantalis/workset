...
func (s *Server) handleWebsocketAttach(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		...
		for {
			...
			if err != nil {
				_ = writeControl(streamCtx, StreamMessage{Type: "error", Error: err.Error()})
				logServerf(
					"ws_control_failed session=%s stream=%s type=%s err=%v",
					req.SessionID,
					streamID,
					controlReq.Type,
					err,
	...
	if len(snapshot.replay) > 0 {
		if err := writeBinary(streamCtx, snapshot.replayNext, snapshot.replay); err != nil {
...
func (s *Server) handleWebsocketControlRequest(
	...
	switch req.Type {
	case "input":
		return nil, session.write(ctx, req.Data)
	case "resize":
