...
func requireAttachReady(t *testing.T, msg StreamMessage) {
	t.Helper()
	if msg.Type != "ready" {
...
func TestAttachMarksTruncatedReplayWhenBufferFallsBehindHistory(t *testing.T) {
	opts := DefaultOptions()
	...
	if second.NextOffset != int64(len(chunkA)+len(chunkB)+len(chunkC)) {
		t.Fatalf(
			"expected replay next offset %d, got %d",
			len(chunkA)+len(chunkB)+len(chunkC),
			second.NextOffset,
		)
	}
...
func TestAttachPrefersTranscriptTailWhenItCoversReplayHistory(t *testing.T) {
	opts := DefaultOptions()
	opts.BufferBytes = 64 * 1024
...
func startTestServerWithOptionsAndServer(t *testing.T, mutate func(*Options)) (*Client, *Server, func()) {
	...
	t.Setenv("TMPDIR", "/tmp")
	tmp := t.TempDir()
	socketPath := filepath.Join(tmp, "terminal-service.sock")
	opts := DefaultOptions()
	...
	for {
		...
		case err := <-errCh:
			if err != nil {
				t.Fatalf("terminal service listen failed: %v", err)
			}
			t.Fatalf("terminal service stopped before socket was ready")
		default:
		}
		...
		if time.Now().After(deadline) {
			t.Fatalf("terminal service socket not ready")
		}
