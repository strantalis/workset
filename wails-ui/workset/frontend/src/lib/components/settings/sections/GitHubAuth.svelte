<script lang="ts">
  ...
  let login = $derived(info?.status.login ?? '')
  let cliInstalled = $derived(info?.cli.installed ?? false)
  ...
  const selectMode = async (next: 'cli' | 'pat'): Promise<void> => {
    ...
    try {
      ...
      statusMessage = next === 'cli' ? 'Using GitHub CLI for authentication.' : 'Using a personal access token.'
    } catch (err) {
  ...
  const saveCLIPath = async (): Promise<void> => {
    ...
    try {
      ...
      statusMessage = info?.cli.installed ? 'GitHub CLI path saved.' : 'Path saved. GitHub CLI still not detected.'
    } catch (err) {
    ...
    {#if authMode === 'cli'}
      <div class="cli-block">
        ...
        <p class="instructions">
          Run the command in your terminal, then refresh status here.
        </p>
        <div class="actions">
        ...
        {#if !cliInstalled}
          <Alert variant="warning">GitHub CLI is not installed. Provide the `gh` binary path to continue.</Alert>
          <div class="cli-path">
            ...
            />
            <div class="actions">
              <Button variant="primary" onclick={saveCLIPath} disabled={saving || cliPath.trim() === ''}>
                {saving ? 'Saving…' : 'Save path'}
              </Button>
              <Button variant="ghost" onclick={browseCLIPath} disabled={saving}>
                Browse…
              </Button>
            </div>
          </div>
