type TerminalInputOrchestratorDeps = {
	...
	markActivity?: (workspaceId: string) => void;
...
export const createTerminalInputOrchestrator = (deps: TerminalInputOrchestratorDeps) => {
	...
	const dispatchInput = (id: string, data: string, bypassInputGuards: boolean): void => {
		if (!data) {
			return;
		}
	...
	const sendInput = (id: string, data: string): void => {
		dispatchInput(id, data, false);
	};

	const sendProtocolResponse = (id: string, data: string): void => {
		dispatchInput(id, data, true);
	};
