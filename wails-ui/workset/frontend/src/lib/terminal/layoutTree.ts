/**
 * Pure layout-tree helpers extracted from TerminalWorkspace to reduce file
 * complexity and line count.  Every function here is side-effect free and
 * operates only on the types defined below.
 */
...
const normalizePaneNode = (node: TerminalLayoutNodeType): PaneNode | null => {
	...
	const activeTabId =
		typeof node.activeTabId === 'string' && tabs.some((tab) => tab.id === node.activeTabId)
			? node.activeTabId
			: tabs[0].id;
	const pane: PaneNode = { id: coerceId(node.id), kind: 'pane', tabs, activeTabId };
	if (typeof node.workspaceId === 'string' && node.workspaceId) {
		pane.workspaceId = node.workspaceId;
	}
	if (typeof node.workspaceName === 'string' && node.workspaceName) {
		pane.workspaceName = node.workspaceName;
	}
	return pane;
};

const normalizeSplitNode = (
	node: TerminalLayoutNodeType,
	recurse: (n: TerminalLayoutNodeType | null | undefined) => LayoutNode | null,
): LayoutNode | null => {
	...
	const ratio =
		...
		node.ratio < 1
			? node.ratio
			: 0.5;
	return { id: coerceId(node.id), kind: 'split', direction, ratio, first, second };
};

export const normalizeNode = (
	node: TerminalLayoutNodeType | null | undefined,
): LayoutNode | null => {
	if (!node || typeof node !== 'object') return null;
	if (node.kind === 'pane') return normalizePaneNode(node);
	if (node.kind === 'split') return normalizeSplitNode(node, normalizeNode);
	return null;
};
...
export const normalizeLayout = (
	...
	if (!candidate || candidate.version !== layoutVersion) return null;
	const root = normalizeNode(candidate.root);
	if (!root) return null;
	return { version: layoutVersion, root, focusedPaneId: candidate.focusedPaneId };
};

// ── Tree traversal / mutation ───────────────────────────────────────────

export const collectTabs = (node: LayoutNode, tabs: TerminalTab[] = []): TerminalTab[] => {
	if (node.kind === 'pane') return tabs.concat(node.tabs);
...
export const splitPane = (
	...
	if (node.kind === 'pane') {
		...
		return { id: newId(), kind: 'split', direction, ratio: 0.5, first: node, second: pane };
	}
...
export const ensureFocusedPane = (next: TerminalLayout): TerminalLayout => {
	if (!next.focusedPaneId) return { ...next, focusedPaneId: firstPaneId(next.root) };
	if (collectPaneIds(next.root).includes(next.focusedPaneId)) return next;
	...
	if (node.kind === 'pane') {
		const tabs = node.tabs
			.map((tab) => {
				...
				if (fix?.terminalId) return { ...tab, terminalId: fix.terminalId };
				return tab;
			})
...
export const buildPane = (
	...
	overrideWorkspaceName?: string,
): PaneNode => {
	const pane: PaneNode = { id: newId(), kind: 'pane', tabs: [tab], activeTabId: tab.id };
...
// ── Keyboard / pane-position helpers ────────────────────────────────────
